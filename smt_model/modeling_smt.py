...
class PositionalEncoding1D(nn.Module):
    def __init__(self, dim: int, len_max: int):
        super().__init__()
        ...
        self.register_buffer(
            "pe",
            torch.zeros((len_max, dim), requires_grad=False),
            persistent=False,
        )
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model: int, num_heads: int, dropout: float = 0.1, bias: bool = True):
        super().__init__()
        assert d_model % num_heads == 0, logger.error(
            "The embeddings depth must be divisible by the number of heads"
        )
        ...
        if not self.has_flash_attn:
            logger.warning(
                "This program cannot run Flash Attention, for optimal computing, "
                "check your GPU driver and your PyTorch version"
            )
    ...
    def forward(
        ...
        if use_flash_attn:
            ...
            attn_output, attn_weights = self._compute_regular_attention(
                q,
                k,
                v,
                key_padding_mask,
        ...
        return (output, attn_weights) if return_weights else (output, None)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x = self.norm_layers[2](x + self.dropout_layers[2](ffn_output))
...
class DecoderStack(nn.Module):
    def __init__(self, num_dec_layers: int, d_model: int, dim_ff: int, num_heads: int, dropout: float):
        super().__init__()
    ...
    def forward(
        ...
        output = x
        all_weights = {"self_attn": [], "cross_attn": []}
        ...
        return (output, all_weights) if return_weights else (output, None)
...
class SMTModelForCausalLM(PreTrainedModel):
    ...
    def forward_decoder(
        ...
        key_target_mask = self._generate_token_mask(
            [lp.shape[0] for lp in last_predictions],
    ...
    def forward(self, encoder_input: torch.Tensor, decoder_input: torch.Tensor, labels=None) -> SMTOutput:
    ...
    def _generate_token_mask(self, token_len, total_size, device: torch.device) -> torch.Tensor:
    ...
    def _generate_causal_mask(self, token_len: int, device: torch.device) -> torch.Tensor:
        return torch.triu(torch.ones(token_len, token_len, dtype=torch.bool, device=device), diagonal=1)
